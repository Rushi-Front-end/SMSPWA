...
import { toastSuccess } from '../../../components/common/tostify/tostify'
...
const SubjectTable = ({ sendSubDataToParent }) => {
    ...
    const subjectTabData = useSelector((state) => state.subjectData)
    const subjectPostRes = useSelector((state) => state.subjectData.postRes)
    const subjectDeleteRes = useSelector((state) => state.schoolData.deleteRes)

    const isLoading = useSelector((state) => state.subjectData.isLoading);

    console.log(subjectTabData, 'subjectTabData')
    ...
    const deleteDatahandler = (data)=>{
        ...
        setTimeout(()=>{
            toast.success("DataDeleted Successfuly",{
                position: "top-right",
                autoClose: 3000,
            })
                        ...
                        <tbody>
                            {
                                isLoading || spinner ? (<Loader />) :
                                    Array.isArray(subjectTabData?.list) && subjectTabData.list.length > 0 ? (
                                        subjectTabData.list.map((dt, index) => {
                                       return     <tr key={dt.id} className="border-b border-defaultborder">
                                                <td>{index + 1}</td>
         ...
         text-[.9375rem]">
                                                        <div className="ti-dropdown hs-dropdown">
                                                            ...
                                                            <ul className="hs-dropdown-menu ti-dropdown-menu hidden">
                                                                ...
                                                                <li><Link className="ti-dropdown-item" to='#' data-hs-overlay={`#hs-vertically-centered-modal-${dt.id}`} onClick={()=>openDelete(dt.id)}>Delete</Link></li>
             ...
             {/* Modal popup Start */}
             <div id={`hs-vertically-centered-modal-${deleteData}`} className="hs-overlay hidden ti-modal">
                <div className="hs-overlay-open:mt-7 ti-modal-box mt-0 ease-out min-h-[calc(100%-3.5rem)] flex justify-center items-center">
                  <div className="ti-modal-content">
                    <div className="ti-modal-header">
                      ...
                      <button type="button" className="hs-dropdown-toggle ti-modal-close-btn" data-hs-overlay={`#hs-vertically-centered-modal-${deleteData}`} >
                    ...
                    <div className="ti-modal-footer">
                      <button type="button" className="hs-dropdown-toggle ti-btn yoti-btn-secondary-full" data-hs-overlay={`#hs-vertically-centered-modal-${deleteData}`}>
                        No
                      </button>
                      <button type="button" className="hs-dropdown-toggle ti-btn ti-btn-primary-full" data-hs-overlay={`#hs-vertically-centered-modal-${deleteData}`} onClick={()=>deleteDatahandler(deleteData)}>
                        Yes
                      </button>
