...
const Reports = () => {
    ...
    const onReportTypeChange = (option) => {
        console.log(option.value,"REPPPP")
       // alert(option.value)
  ...
  const onSubmit = async (formData) => {
    if (isValid) {
        try {
            ...
            setReportHead({
                ...formData, // Spread the formData to include other fields
                prakalpName, // Add prakalpName
            ...
            switch (reportType) {
                ...
                case 'Expense Report':
                    ...
                    break;
                case 'Hostel Attendance Report':
                    url = new URL('https://sms-webapi-hthkcnfhfrdcdyhv.eastus-01.azurewebsites.net/api/Report/GetHostelAttendanceReports');
                    break;
                case 'Student Health Report':
                    url =  new URL('https://sms-webapi-hthkcnfhfrdcdyhv.eastus-01.azurewebsites.net/api/Report/GetStudentHealthReports');
                    break;
                case 'Student Attendance Report':
                    url =  new URL('https://sms-webapi-hthkcnfhfrdcdyhv.eastus-01.azurewebsites.net/api/Report/GetSchoolStudentAttenReports');
                   break;
                case 'Staff Attendance Report':
                    url =  new URL('https://sms-webapi-hthkcnfhfrdcdyhv.eastus-01.azurewebsites.net/api/Report/GetStaffAttenReports');
                   break;
            ...
            console.log(reportType, "Current Report Type");
            console.log(url.toString(), "Constructed URL for Fetch");
            // Fetch report data
            ...
            {modalIsOpen && (
                <div id="hs-vertically-centered-modal" className="hs-overlay reports-modal-popup ti-modal">
                    <div className="hs-overlay-open:mt-7 ti-modal-box mt-0 ease-out min-h-[calc(100%-3.5rem)] flex items-center">
                        ...
                        {reportName == 'Hostel Attendance Report' && <HostelReport reportGenData={reportGenData} /> }
                        {reportName == 'Student Health Report' && <StudentHealthReport reportGenData={reportGenData} />}
                        {reportName == 'Student Attendance Report' && <StudentAttendanceReport reportGenData={reportGenData} />}
                        {reportName == 'Staff Attendance Report' && <StaffAttendanceReport reportGenData={reportGenData} />}
                            </div>
